...
- **`lambda-envs/memory_games_sac.py`**: matched replay-learning pilots for rank-matching Concentration (52 cards, 13 ranks) and partial Minesweeper (6×6, six mines). Self-contained gymnax envs in `_concentration_env.py` / `_minesweeper_env.py`; no POPGym install required. `--method sac` removes λ branches, `critics` retains their regression losses with discrepancy coefficient zero, `ld` adds `--lambda-coef`. Default H128 GRU, batch32, sequence32, tail32, full-episode burn-in (104 / 30), α0.01, hard main-critic backup; previous executed action is appended by an env-state adapter so it and the recurrent carry persist across training rounds. Concentration caches transient failed-pair reveals; Minesweeper has a distinct reset marker and all possible clue-count bins. Neither masks actions. Greedy + sampled returns, history-only decision diagnostics, random/scripted references, synchronized throughput, JSONL/W&B and exact resumable checkpoints. `--remember --memory-type identity` exposes previously seen information only as a history-table control. Cluster array runs both envs × `{sac,critics,ld0.01,ld0.1}`, 3 seeds/condition, 100k steps. Measured RTX3090 training projections: 3 seeds 100k takes 13/10 min; 10 seeds 1M takes 5.1/3.7 h (Concentration/Minesweeper), excluding eval/checkpoint overhead. These are throughput measurements, not positive learning results. Details and launch/resume commands: `examples/lambda-envs/memory_games.md`. Checks: `tests/test_concentration_env.py`, `tests/test_minesweeper_env.py`, `tests/test_memory_games_runner.py` (143 tests).
