...
- **Prev-action input (`use_prev_action` / packed carry)**: when `create_iqlearn(_from_env)` is built with `use_prev_action=True`, the FE carry is the full agent recurrent state `[memory_carry | prev_action_enc(action_dim)]` — one-hot for discrete, squashed action values for continuous. The encoding is concatenated with the flattened observation *before* the projection (matching `ActionConcatWrapper` in the original lambda-discrepancy code, where the action enters through the embedding, never the cell directly). `carry_dim` grows by `action_dim`; hand-rolled `CARRY_DIM` formulas in examples must add it when the flag is on. A zero tail means "no previous action", so every carry-zeroing point (done resets, window starts, `zero_carry()` helpers) resets the action input for free. `predict` writes its chosen action into the returned carry's tail — callers that execute a *different* action (e.g. epsilon-greedy overrides) must rewrite it via `RecurrentFeatureExtractor.write_prev_action`. Tests: `tests/test_prev_action.py` (includes a bit-exact equivalence test against the obs-concat formulation).
