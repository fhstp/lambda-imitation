...
def create_iqlearn_from_env(
    ...
    * Constructing three :class:`~lambda_imitation.iqlearn.MLPFeatureExtractor` instances
    ...
    if memory_factory is not None:
        ...
        critic_q2_mem = memory_factory(fe_out_dim, nnx.Rngs(int(seeds[7])))
        mc_critic_q1_mem = memory_factory(fe_out_dim, nnx.Rngs(int(seeds[8])))
        mc_critic_q2_mem = memory_factory(fe_out_dim, nnx.Rngs(int(seeds[9])))
    else:
        actor_mem = None
        critic_q1_mem = None
        critic_q2_mem = None
    ...
    # One FE + one memory serve all networks: actor, SAC cq1/cq2, and MC cq1/cq2.
    shared_fe = None
    shared_mem = None
    if use_shared_trunk:
        if fe_hidden_dims is None:
