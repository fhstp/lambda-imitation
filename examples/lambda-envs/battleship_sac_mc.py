"""SAC online training demo: Battleship (discrete action space, lambda-envs).
...
Battleship is a sequential decision problem: the agent fires at a hidden grid
to sink all ships.  Observations contain only the last hit/miss result and a
mask of already-fired cells, making it partially observable.
...
Usage
-----
    python battleship_sac_mc.py                  # 5×5 grid (default)
    python battleship_sac_mc.py --grid-size 3    # smallest 3×3 grid
    python battleship_sac_mc.py --grid-size 10   # full 10×10 grid
...
parser = argparse.ArgumentParser(
    ...
    description="SAC + MC critic on Battleship (discrete, lambda-envs).",
)
parser.add_argument(
    "--grid-size",
    type=int,
    ...
    choices=[3, 5, 10],
    metavar="{3,5,10}",
    help="grid size: 3 (3×3), 5 (5×5), or 10 (10×10) (default: 5)",
)
parser.add_argument(
    "--rounds",
    type=int,
...
parser.add_argument(
    ...
    default=0.3,
    metavar="TE",
    help="target entropy for alpha autotuning (default: 0.3)",
)
parser.add_argument(
    "--burn-in-len",
    type=int,
...
rows, cols, ship_lengths = _GRID_CONFIGS[args.grid_size]

env = Battleship(rows=rows, cols=cols, ship_lengths=ship_lengths)
env_params = env.default_params
spec = env_spec_from_gymnax(env, env_params)
# obs_shape=(1 + rows*cols,), action_dim=rows*cols, is_discrete=True

env_label = f"battleship_{args.grid_size}x{args.grid_size}"
...
print(f"Building SAC agent for {env_label} (action_dim={spec.action_dim})…")
