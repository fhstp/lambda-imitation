...
# Everything that is not Battleship-specific (training, losses, offline/expert
# prefill, PER, probe, I/O, W&B) lives in _probe_common so the PocMan probe
# script offers the exact same interface.
common.add_common_args(
    parser, output_dir_default="./battleship_probe_output",
    wandb_project_default="offline-lambda-battleship-results")

args = common.parse_args(parser, extra_flags_env="BATTLESHIP_PROBE_EXTRA_FLAGS")
...
# ── probe MLP (shared with the PocMan probe) ─────────────────────────────────
...
if not args.vis_only:
    ...
    _stack_states = common.stack_states
    _unstack_state = common.unstack_state
    _split_each = common.split_each
    _agg = common.agg
    _ep_bounds = common.episode_bounds

    _transplant_fe, _save_fe = common.make_fe_checkpointer(
        args.init_fe, args.output_dir)
    ...
    evaluate_critic = (
        common.make_evaluate_critic(debug_fns, env, env_params,
                                    num_actions=N, mask_fn=mask_fn, **_eval_kwargs)
    ...
    # The probe has 2N outputs: first N = P(ship), last N = P(cell was fired at).
    ...
    _train_probe, _train_probe_v, _probe_train_chunk = common.make_probe_trainer(
        optax.adam(args.probe_lr), carry_dim=CARRY_DIM, n_out=2 * N,
        hidden=args.probe_hidden_dim, batch_size=args.probe_batch_size,
        tqdm=tqdm, wandb=_wandb)

    _AGE_BUCKETS = common.AGE_BUCKETS
    _cell_ages = common.cell_ages
    _probe_metrics_from_probs = partial(common.probe_metrics_from_probs, n_cells=N)
    _probe_metrics = partial(common.probe_metrics, n_cells=N)
    ...
    if args.num_seeds > 1:
        ...
        def run_group(group_idx, gidxs):
            ...
            def round_eval(keys, batched):
                ...
                evals = {"return": np.array(returns), "steps_to_clear": np.array(steps),
                         "cleared_frac": np.array(cleared), "cg": None}
                if evaluate_critic is not None:
                    keys, cg_keys = _split_each(keys)
                    cgr, cgs, cgc, qsp, qrg = _evaluate_critic_v(batched, cg_keys, 10)
                    evals["cg"] = (np.array(cgr), np.array(cgs), np.array(cgc),
                                   np.array(qsp), np.array(qrg))
                return keys, evals

            def on_round(rnd, step, evals, metrics):
                returns = evals["return"]; steps = evals["steps_to_clear"]
                cleared = evals["cleared_frac"]; cg = evals["cg"]
                for j, gi in enumerate(gidxs):
            ...
            batched, keys = common.run_seed_group(
                ...
                qpi=common.Hooks(
                    lambda b, rnd: _qpi_collect_host(_unstack_state(b, 0), gidxs[0], rnd),
                    _qpi_render_host),
                qpi_interval=_qpi_interval,
