...
* ``remember=False``: float32 one-hot vector of size K+1. Indices 0..K-1
...
* ``remember=True``: N consecutive row-major one-hot vectors of size K+1,
  followed by ``min(timestep / S, 1)``. Each cell encodes its observed clue
  or, in slot K, "unviewed". Unseen safe squares and mines are indistinguishable.
  This is an explicit observation-history control, not a hidden-board oracle.
  Its dimension is N*(K+1)+1; the time feature counts repeats as well as reveals.

Thus 6x6/6 has observation dimensions 8 / 289, and the small 4x4/2 smoke
configuration has 4 / 65. ``get_obs`` reconstructs exactly the observation
...
class MineSweeper(environment.Environment):
    """Single-query Minesweeper; use ``remember=True`` for the history control.

    ``reset`` and ``step`` are inherited unchanged from gymnax. Use
    ``step_env`` to inspect a terminal board before auto-reset. Params can
    override the timeout; reward scales and the history time normalization
    stay tied to the board's default ``episode_length``.
    """

    def __init__(self, rows=6, cols=6, num_mines=6, remember=False):
        ...
        self.obs_requires_prev_action = not self.remember
    ...
    def get_obs(self, state, params=None):
        if self.remember:
            clues = jnp.where(state.viewed, state.neighbor_counts, self.num_clues)
            table = jax.nn.one_hot(clues, self.num_clues + 1, dtype=jnp.float32)
            time = jnp.minimum(state.timestep / self.episode_length, 1.0)
            return jnp.concatenate((table.reshape(-1), jnp.asarray(time, jnp.float32)[None]))
